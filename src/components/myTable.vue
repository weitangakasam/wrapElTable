...
<div>
  <div >
     <el-input style="float:right;width:240px" v-if="table_config.showSearch"
          :value="searchPam"
          @input="search"
          size="mini"
          placeholder="输入关键字搜索"/>
     <el-table
      ...
      >
      <el-table-column v-if="table_config.multiSelect" type="selection"></el-table-column>
        <!--这里外面用template包裹是因为把v-for和v-if写在一起性能比较差-->
        <template  v-for="ele in table_config.thead">
           <el-table-column  :key="ele.key" v-if="ele.type==='function'"
              :prop=ele.prop
              :label=ele.lable
               width="180"> 
            ...
            <el-table-column v-else-if="ele.type === 'slot'" 
            :key="ele.key"
             :prop=ele.prop
              :label=ele.lable>
  ...
  computed:{
    showData(){
      ...
      let data = this.data.slice(prevNum, prevNum + this.table_config.pageSize)
       let {searchProp} = this.table_config
      if(searchProp && searchProp.length > 0){
          data = data.filter(ele=>{
            let reg = new RegExp(this.searchPam)
